...
public class CacheProviderRepository implements ProviderRepository {
    ...
    public CacheProviderRepository(RestProviderDAO restProviderService, @Value("${kakka.provider.cache.refresh.max.size:1000}") int cacheMaxSize) {
        this.restProviderService = restProviderService;
        this.cacheMaxSize = cacheMaxSize;
        this.cache = CacheBuilder.newBuilder().maximumSize(cacheMaxSize).build();
    }

    @Scheduled(fixedRateString = "${kakka.provider.cache.refresh.interval:300000}")
