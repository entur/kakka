...
package no.entur.kakka.task.routes.kartverket;
...
public class AdministrativeUnitsDownloadRouteBuilder extends BaseRouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        singletonFrom("quartz://kakka/administrativeUnitsDownload?cron=" + cronSchedule + "&trigger.timeZone=Europe/Oslo")
                ...
                .setBody(constant(TaskConstants.KARTVERKET_ADMINISTRATIVE_UNITS_DOWNLOAD))
                .to(ExchangePattern.InOnly,"direct:taskStart")
                .routeId("admin-units-download-quartz");

        from(TaskConstants.KARTVERKET_ADMINISTRATIVE_UNITS_DOWNLOAD.getEndpoint())
                .log(LoggingLevel.INFO, "Start downloading administrative units")
                .process(e -> JobEvent.systemJobBuilder(e).startTask(TaskType.ADMINISTRATIVE_UNITS_DOWNLOAD).build()).to("direct:updateStatus")
                ...
                .setProperty(TaskConstants.NEXT_TASK, constant(TaskConstants.TIAMAT_ADMINISTRATIVE_UNITS_UPDATE_START))
                .otherwise()
