...
package no.entur.kakka.task.routes.tiamat;


import no.entur.kakka.task.BaseRouteBuilder;
import no.entur.kakka.task.TaskConstants;
import no.entur.kakka.task.netex.TopographicPlaceConverter;
...
public class TiamatAdministrativeUnitsUpdateRouteBuilder extends BaseRouteBuilder {
    ...
    public void configure() throws Exception {
        ...
        singletonFrom("quartz://kakka/tiamatAdministrativeUnitsUpdate?cron=" + cronSchedule + "&trigger.timeZone=Europe/Oslo")
                ...
                .setBody(constant(TaskConstants.TIAMAT_ADMINISTRATIVE_UNITS_UPDATE_START))
                .to(ExchangePattern.InOnly, "direct:taskStart")
                .routeId("tiamat-admin-units-update-quartz");

        from(TaskConstants.TIAMAT_ADMINISTRATIVE_UNITS_UPDATE_START.getEndpoint())
                .log(LoggingLevel.INFO, "Starting update of administrative units in Tiamat")
                .process(e -> JobEvent.systemJobBuilder(e).startTask(TaskType.TIAMAT_ADMINISTRATIVE_UNITS_UPDATE).build()).to("direct:updateStatus")
