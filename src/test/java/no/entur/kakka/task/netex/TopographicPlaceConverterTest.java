...
package no.entur.kakka.task.netex;
...
import no.entur.kakka.task.geojson.GeojsonFeatureWrapperFactory;
import no.entur.kakka.task.nabu.rest.AdministrativeZone;
import no.entur.kakka.task.netex.geojson.GeoJsonSingleTopographicPlaceReader;
import no.entur.kakka.task.netex.sosi.SosiTopographicPlaceReader;
import no.entur.kakka.task.sosi.SosiElementWrapperFactory;
...
public class TopographicPlaceConverterTest {
    ...
    public void testCovertWOFCountriesToGeoJson() throws Exception {
        final List<AdministrativeZone> wof = new GeoJsonSingleTopographicPlaceReader(new GeojsonFeatureWrapperFactory(null),
                new File("src/test/resources/no/entur/kakka/task/geojson/finland.geojson")).read()
                .stream()
    ...
    public void testConvertAdminUnitsFromSosi() throws Exception {
        TopographicPlaceReader reader = new SosiTopographicPlaceReader(new SosiElementWrapperFactory(), List.of(new File("src/test/resources/no/entur/kakka/task/sosi/SosiTest.sos")));
    ...
    public void testConvertNeighbouringCountriesFromGeoJson() throws Exception {
        TopographicPlaceReader reader = new GeoJsonSingleTopographicPlaceReader(new GeojsonFeatureWrapperFactory(null),
                new File("src/test/resources/no/entur/kakka/task/geojson/finland.geojson"));
