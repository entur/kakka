    ...
    public Collection<ElasticsearchCommand> transform(InputStream gtfsStream, @Header(value = Constants.WORKING_DIRECTORY) String workingDirPath, @Header(value = Constants.FILE_HANDLE) String fileName) {
        ...
        if (!workingDir.exists()) {
            try {
                ...
                throw new KakkaException("Unable to create working directory " + workingDirPath + ": " + ioe.getMessage(), ioe);
            }
        }
        File gtfsFile;
        try {
            ...
            throw new KakkaException("Unable to create tmp file for gtfs: " + ioe.getMessage(), ioe);
        }
